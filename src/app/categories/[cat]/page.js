...
function items(loc,cat){
  ...
  const pr=getProjects(loc).filter(p=>p.category===cat).map(p=>({type:"projects",slug:p.slug,href:(loc==="en"?"":"/"+loc)+"/projects/"+p.slug,title:p.name,img:projectImages(p.slug).hero,alt:p.heroAlt,meta:null}));
  return [...arts,...pr];
}
