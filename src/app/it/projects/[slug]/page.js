...
export function generateMetadata({params}){const p=findProject(LOC,params.slug);if(!p)return{};return{title:`${p.name} - VersionDude`,description:p.excerpt,openGraph:{title:p.name,description:p.excerpt,images:[projectImages(p.slug).hero]},robots:{index:true,follow:true},alternates:{canonical:`https://versiondude.net/${LOC}/projects/${p.slug}`}};}
